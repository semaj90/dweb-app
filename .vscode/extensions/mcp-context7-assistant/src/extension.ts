    ...
    .then((isHealthy) => {
      if (isHealthy) {
        ...
        statusBarManager.updateStatus(
          "ready",
          "Context7 MCP + Enhanced RAG Ready"
        );
      } else {
        console.log("❌ Enhanced RAG Backend is not available");
        statusBarManager.updateStatus(
          "ready",
          "Context7 MCP Ready (RAG Offline)"
        );
      }
    })
    .catch((error) => {
...
function registerCommands(context: vscode.ExtensionContext) {
  ...
  const analyzeContextCommand = vscode.commands.registerCommand(
    ...
    async () => {
      try {
        ...
        const suggestions =
          await contextAnalyzer.getContextAwareSuggestions(vsCodeContext);
        ...
        if (activeEditor) {
          ...
          const ragResult = await mcpServerManager.callMCPTool(
            "enhanced_rag_query",
            {
        ...
        statusBarManager.updateStatus(
          "ready",
          `Enhanced analysis: ${suggestions.length} insights`
        );
  ...
  const enhancedRAGQueryCommand = vscode.commands.registerCommand(
    "mcp.enhancedRAGQuery",
    async () => {
      try {
        const query = await vscode.window.showInputBox({
          prompt: "Enter your enhanced RAG query",
          placeHolder:
            'e.g., "How to implement SvelteKit forms with Drizzle ORM?"',
        });
  ...
  const agentOrchestrationCommand = vscode.commands.registerCommand(
    "mcp.agentOrchestration",
    async () => {
      try {
        ...
        if (agentType) {
          const prompt = await vscode.window.showInputBox({
            ...
            placeHolder:
              'e.g., "Analyze this codebase for performance improvements"',
          });

          if (prompt) {
            statusBarManager.updateStatus(
              "processing",
              `Orchestrating ${agentType} agent...`
            );

            const result = await mcpServerManager.callMCPTool(
              `agent_orchestrate_${agentType}`,
              {
                prompt,
  ...
  const memoryGraphCommand = vscode.commands.registerCommand(
    "mcp.memoryGraph",
    async () => {
      try {
        ...
        if (action) {
          ...
          switch (action) {
            case "Read Graph":
              result = await mcpServerManager.callMCPTool(
                "mcp_memory2_read_graph",
                {}
              );
              break;
            case "Search Nodes":
  ...
  const enhancedMetricsCommand = vscode.commands.registerCommand(
    "mcp.enhancedMetrics",
    async () => {
      try {
        const metrics = mcpServerManager.getEnhancedMetrics();
...
async function detectProjectTypeAndStack(
  workspaceRoot: string
): Promise<{ projectType: ProjectType; detectedStack: TechStack }> {
  try {
...
function generateStackReport(
  ...
  stack: TechStack,
  mcpSuggestions: Array<{ library: string; topics: string[] }>
): string {
