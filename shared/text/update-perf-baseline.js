// JS runtime script to update rolling performance baseline (avoids TS loader needs)
// Only updates when ALLOW_BASELINE_UPDATE=true and (branch main/master or unspecified)
...
function runSplitterBenchmark(iterations = 50) {
  // Inline minimal benchmark (duplicated from benchmark-splitter to stay JS-safe)
  const { performance } = require('node:perf_hooks');
  const { getSharedLegalSentenceSplitter } = require('./legal-sentence-splitter.js');
  ...
  for (const t of texts) splitter.split(t);
