...
public:
    ...
    std::vector<float> compute_batch_cosine_similarity(
        ...
        std::cout << "Batch similarity computation: " << vectors_a.size() 
                  ...
                  << " (" << (total_time / vectors_a.size()) << "ms per pair)" << std::endl;
...
private:
    ...
    json get_performance_metrics() {
        ...
        perf["averageProcessingTimeMs"] = metrics.operations_processed > 0 ? 
            metrics.total_processing_time / metrics.operations_processed : 0.0f;
        perf["maxProcessingTimeMs"] = metrics.max_processing_time;
        perf["minProcessingTimeMs"] = metrics.min_processing_time == std::numeric_limits<float>::max() ? 
            0.0f : metrics.min_processing_time;
