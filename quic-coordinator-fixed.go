...
func (qc *QUICCoordinator) Start() error {
	...
	qc.logger.Printf("QUIC coordinator started on %s", addr)
...
func (qc *QUICCoordinator) acceptConnections() {
	for {
		...
		default:
			...
			if err != nil {
				qc.logger.Printf("Failed to accept connection: %v", err)
				continue
			}
...
func (qc *QUICCoordinator) handleConnection(conn quic.Connection) {
	...
	qc.logger.Printf("New QUIC connection: %s from %s", clientID, conn.RemoteAddr())
	...
	qc.logger.Printf("QUIC connection closed: %s", clientID)
}
...
func (qc *QUICCoordinator) handleStreams(qconn *QUICConnection) {
	for {
		...
		default:
			...
			if err != nil {
				qc.logger.Printf("Failed to accept stream: %v", err)
				continue
			}
...
func (qc *QUICCoordinator) handleStream(stream quic.Stream, qconn *QUICConnection) {
	...
	if err != nil {
		qc.logger.Printf("Failed to read message type: %v", err)
	...
	if !exists {
		qc.logger.Printf("No handler for message type: %s", msgType)
	...
	if err != nil {
		qc.logger.Printf("Stream handler error: %v", err)
		atomic.AddInt64(&qconn.metrics.ErrorCount, 1)
		atomic.AddInt64(&qc.totalErrors, 1)
...
func main() {
	logger := log.New(log.Writer(), "[QUIC] ", log.LstdFlags)
	config := &QUICConfig{
		Address:          "0.0.0.0",
		Port:            9443,
		MaxStreams:      1000,
	...
	log.Println("QUIC coordinator is running. Press Ctrl+C to stop.")
