...
class LegalAIWindowsService extends EventEmitter {
  constructor(options = {}) {
    ...
    this.config = {
      ...
      processes: {
        ...
        'perf-monitor': {
          script: './perf-monitor.exe',
          cwd: path.resolve(__dirname, '../go-microservice/bin'),
          env: { PERF_HEAP_WARN_MB: '600', PERF_GC_PAUSE_WARN_MS: '250' },
          restart: true,
          maxRestarts: 5
        },
        'node-cluster': {
  ...
  async startAllProcesses() {
    ...
    try {
      ...
      const startOrder = [
        'perf-monitor',       // Metrics first so other services can report early
  ...
  performHealthChecks() {
    ...
    // Lightweight HTTP health probe for perf-monitor service
    if (this.processes.has('perf-monitor')) {
      const http = require('http');
      const req = http.get('http://127.0.0.1:8098/health', (res) => {
        if (res.statusCode !== 200) {
          console.warn('[LEGAL-AI-SERVICE] perf-monitor unhealthy status code:', res.statusCode);
          if (this.config.processes['perf-monitor'].restart) {
            this.restartManagedProcess('perf-monitor');
          }
        }
      });
      req.on('error', () => {
        console.warn('[LEGAL-AI-SERVICE] perf-monitor not responding, attempting restart');
        if (this.config.processes['perf-monitor'].restart) {
          this.restartManagedProcess('perf-monitor');
        }
      });
