...
class AppLogger {
  ...
  log(level: LogEntry['level'], message: string, metadata?: Partial<LogEntry>) {
    ...
    if (dev) {
      console.log(`[${entry.level.toUpperCase()}] ${entry.timestamp}: ${message}`, metadata);
    }
...
// Convenience methods
export const logInfo = (message: string, metadata?: any) => logger.log('info', message, metadata);
export const logWarn = (message: string, metadata?: any) => logger.log('warn', message, metadata);
export const logError = (message: string, metadata?: any) => logger.log('error', message, metadata);
export const logDebug = (message: string, metadata?: any) => logger.log('debug', message, metadata);
