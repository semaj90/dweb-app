...
function createFormStore(options: FormOptions = {}) {
	const { initialValues = {}, validators = {}, requiredFields = [], onSubmit } = options;
	...
	return {
			...
			if (canSubmit && onSubmit) {
				try {
					const currentValues = Object.fromEntries(
						Object.values(initialFields).map(field => [field.name, field.value])
					);
		...
		addField: (name: string, initialValue: any = '', isRequired: boolean = false) => {
			update(state => ({
				...state,
				fields: {
					...state.fields,
