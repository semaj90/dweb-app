...
// When building under Emscripten include its headers; otherwise build a native fallback.
#ifdef __EMSCRIPTEN__
#include <emscripten/emscripten.h>
#include <emscripten/bind.h>
#else
// Provide no-op macro so C exports still compile natively.
...
namespace YoRHa {
    ...
    public:
        ...
        std::string processDocument(const std::string& input) {
            ...
            // Neural network confidence inference
        ...
        // YoRHa neural network vector transformation (renamed to avoid overload clash)
        std::vector<float> neuralVectorTransform(const std::vector<float>& input) {
        ...
        // Neural network confidence calculation (renamed to avoid overload clash)
        float computeConfidence(const std::vector<float>& features) {
...
// Emscripten bindings for JavaScript integration (only when compiling with Emscripten)
#ifdef __EMSCRIPTEN__
EMSCRIPTEN_BINDINGS(yorha_neural_processor) {
    emscripten::class_<YoRHa::NeuralProcessor>("YoRHaNeuralProcessor")
        ...
        .function("neuralVectorTransform", &YoRHa::NeuralProcessor::neuralVectorTransform)
        .function("computeConfidence", &YoRHa::NeuralProcessor::computeConfidence)
