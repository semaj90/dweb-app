    ...
    try {
      ...
      if (!options?.dryRun && result.summary.totalIssues > 0) {
        ...
        result.recommendations = [
          ...result.recommendations,
          ...contextualPractices.map((practice: unknown) => `Context7: ${practice}`)
        ];
      }
  ...
  async getAutoFixRecommendations(component?: string): Promise<{
    ...
    try {
      ...
      if (analysis?.recommendations.length) {
        return recommendations.filter((rec: unknown) => analysis.recommendations.some((r: unknown) => r.toLowerCase().includes(rec.area) || 
