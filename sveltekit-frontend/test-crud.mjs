...
async function testCRUDOperations() {
  ...
  try {
    ...
    // Read user
    const readUser = await db.select().from(schema.users).where(eq(schema.users.id, testUser[0].id));
    ...
    // Create case
    const testCase = await db.insert(schema.cases).values({
    ...
    // Create evidence
    const testEvidence = await db.insert(schema.evidence).values({
    ...
    console.log('  ✓ Case with evidence count:', caseWithEvidence.evidence.length);
    console.log('  ✓ Evidence creator:', caseWithEvidence.evidence[0].creator.name);
    ...
    await db.delete(schema.evidence).where(eq(schema.evidence.id, testEvidence[0].id));
