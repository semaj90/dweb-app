...
const LEGAL_ENTITY_PATTERNS = {
  CASE_NUMBER: /(?:Case\s+(?:No\.?|Number)\s*:?\s*)?([A-Z]{1,4}[-\s]?\d{2,4}[-\s]?\d{2,6})/gi,
  COURT_NAME: /(?:in\s+the\s+)?((?:United States|U\.S\.)\s+(?:District|Supreme|Appeals?)\s+Court|(?:State|Superior|Municipal|Family)\s+Court)/gi,
...
class LegalDocumentProcessor {
  ...
  assessRiskFactors(content) {
    ...
    if (!LEGAL_ENTITY_PATTERNS.CASE_NUMBER.test(content) && content.length > 500) {
      riskFactors.push({
  ...
  extractKeyPhrases(content) {
    ...
    for (const sentence of sentences) {
      ...
      if (trimmed.length > 20 && trimmed.length < 200) {
        if (/\b(?:hereby|whereas|therefore|notwithstanding|pursuant to)\b/i.test(trimmed)) {
  ...
  calculateSentimentScore(content) {
    const positiveWords = ['agree', 'consent', 'approve', 'accept', 'comply', 'cooperate'];
    const negativeWords = ['deny', 'reject', 'dispute', 'violation', 'breach', 'fail', 'refuse'];
  ...
  async processDocument(document, analysisType, options = {}) {
    ...
    try {
      ...
      if (options.performCompliance || analysisType === 'FULL_COMPLIANCE_CHECK') {
        results.complianceChecks = this.performComplianceCheck(document.content);
      }

      if (options.includeRiskAssessment || analysisType === 'FULL_COMPLIANCE_CHECK') {
  ...
  generateSummary(results, document) {
    ...
    summaryParts.push(`Analyzed ${document.type || 'legal document'} (${Math.round(document.content.length / 1000)}k characters)`);

    if (results.entities) {
      const entityTypes = [...new Set(results.entities.map(e => e.type))];
      summaryParts.push(`Found ${results.entities.length} legal entities of ${entityTypes.length} types`);
    }

    if (results.complianceChecks) {
      const passedChecks = results.complianceChecks.filter(c => c.passed).length;
      const totalChecks = results.complianceChecks.length;
      summaryParts.push(`Compliance: ${passedChecks}/${totalChecks} checks passed`);
    }

    if (results.riskFactors) {
      const highRisk = results.riskFactors.filter(r => r.severity === 'HIGH').length;
      const mediumRisk = results.riskFactors.filter(r => r.severity === 'MEDIUM').length;
  ...
  try {
    switch (type) {
      case 'ANALYZE_LEGAL_DOCUMENT':
        const result = await processor.processDocument(document, analysisType, options);
        self.postMessage({
      ...
      case 'COMPLIANCE_CHECK':
        const complianceChecks = processor.performComplianceCheck(document.content);
        self.postMessage({
