...
export class CrewAIService {
    ...
    const agents: CrewAIAgent[] = [
      {
        ...
        tools: ['evidence_analyzer', 'witness_interview_tool', 'timeline_builder', 'case_documentation'],
        llmConfig: {
          model: 'gemma3-legal',
          temperature: 0.1,
          maxTokens: 1536,
      ...
      },
      {
        ...
        tools: ['legal_research_tool', 'precedent_finder', 'statute_analyzer', 'case_strategy_builder'],
        llmConfig: {
      ...
      },
      {
        ...
        tools: ['digital_forensics_tool', 'metadata_analyzer', 'authenticity_verifier', 'chain_custody_tracker'],
        llmConfig: {
  ...
  createContractAnalysisCrew(): CrewAICrew {
    const agents: CrewAIAgent[] = [
      ...
      },
      {
        ...
        tools: ['negotiation_analyzer', 'alternative_drafter', 'leverage_assessor', 'strategy_builder'],
        llmConfig: {
          model: 'gemma3-legal',
          temperature: 0.3,
          maxTokens: 1536,
  ...
  async getAvailableTools(): Promise<string[]> {
    try {
      ...
      return [
        'evidence_analyzer', 'legal_research_tool', 'contract_analyzer',
        'witness_interview_tool', 'digital_forensics_tool', 'report_generator',
        'precedent_finder', 'risk_assessor', 'compliance_checker'
      ];
    }
  }
  ...
  async *streamExecution(executionId: string): AsyncGenerator<CrewTaskResult, void, unknown> {
    const eventSource = new EventSource(`${this.baseUrl}/api/execution/${executionId}/stream`);
