...
interface StageStats { samples: number[]; sum: number; count: number; anomalies?: number }
const STAGES: PipelineStage[] = ['gpu','wasm','embedding','retrieval','llm','final'];
const stageData: Record<PipelineStage, StageStats> = STAGES.reduce((acc, s) => { acc[s] = { samples: [], sum: 0, count: 0, anomalies: 0 }; return acc; }, {} as any);
...
export function recordStageLatency(stage: PipelineStage, ms: number) {
    ...
    if (ms < 0) { s.anomalies = (s.anomalies || 0) + 1; return; }
    const sorted = [...s.samples].sort((a, b) => a - b);
    const median = sorted.length ? sorted[Math.floor(sorted.length / 2)] : ms;
    if (sorted.length && ms > 5 * (median || 1)) { s.anomalies = (s.anomalies || 0) + 1; }
    s.samples.push(ms);
