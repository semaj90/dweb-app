...
export class QdrantService {
  ...
  public connectionStatus$: Writable<'connected' | 'disconnected' | 'connecting' | 'error'> =
    writable<'connected' | 'disconnected' | 'connecting' | 'error'>('disconnected');
  public collections$: Writable<string[]> = writable<string[]>([]);
  public searchResults$: Writable<SearchResult[]> = writable<SearchResult[]>([]);
  ...
  public async upsert(collection: string, payload: { id: string; vector: number[]; payload?: any }[]) {
    ...
    const points = payload.map(p => {
      let v = p.vector;
      if (Array.isArray(v)) {
        if (v.length > this.defaultVectorSize) v = v.slice(0, this.defaultVectorSize);
        else if (v.length < this.defaultVectorSize) v = v.concat(Array(this.defaultVectorSize - v.length).fill(0));
      }
      return { id: p.id, vector: v, payload: p.payload };
    });
    await this.client.upsert(collection, { wait: true, points });
    return { status: 'ok', count: points.length, vectorSize: this.defaultVectorSize };
  }

  // Compatibility: searchSimilar -> perform a vector search against a collection
  public async searchSimilar(vector: number[], options: { collection?: string; topK?: number } = {}) {
    if (!this.isConnected) throw new Error('Qdrant not connected');
    const collection = options.collection || QDRANT_COLLECTIONS.documents;
    const limit = options.topK || 10;
    const res = await this.client.search(collection, { vector, limit, with_payload: true });
    return res;
  }
  ...
  private async createCollection(name: string): Promise<void> {
    const vectorSize = this.defaultVectorSize; // model dimension (env configurable)
  ...
  public async addDocument(document: Omit<DocumentVector, 'id' | 'timestamp'>): Promise<string> {
    ...
    // Normalize embedding to expected size
    let norm = document.embedding;
    if (Array.isArray(norm)) {
      if (norm.length > this.defaultVectorSize) norm = norm.slice(0, this.defaultVectorSize);
      else if (norm.length < this.defaultVectorSize) norm = norm.concat(Array(this.defaultVectorSize - norm.length).fill(0));
    }
  ...
  public async searchDocuments(
    ...
    const searchResults: SearchResult[] = results.map((result: any) => ({
      ...
      highlights: this.extractHighlights(
        query,
        result.payload as DocumentVector & Record<string, unknown> & { [key: string]: unknown }
      ),
    }));
  ...
  public async searchLegalDocuments(
    ...
    return results.map((result: any) => ({
      ...
      payload: result.payload as LegalDocumentVector &
        ...
        Record<string, unknown>,
      highlights: this.extractLegalHighlights(
        result.payload as LegalDocumentVector & Record<string, unknown> & Record<string, unknown>
      ),
    }));
  }
  ...
  private async generateLegalAutoTags(
    content: string,
...
Return a JSON object with this enhanced structure:
{
  ...
  private async storeLegalTagPredictions(
    documentId: string,
...
Return a JSON object with this structure:
{
  ...
  private async storeTagPredictions(
    documentId: string,
    predictions: TagPrediction[]
  ): Promise<void> {
    const points: PointStruct[] = predictions.map((prediction, index) => ({
