...
class IndexedDBService {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains('documents')) {
          ...
          documentsStore.createIndex('lastUpdated', 'lastUpdated', { unique: false });
        ...
        if (!db.objectStoreNames.contains('userInteractions')) {
          ...
          interactionsStore.createIndex('documentId', 'documentId', { unique: false });
        }
  ...
  async getDocumentsByType(type: 'case' | 'evidence' | 'document'): Promise<CachedDocument[]> {
    if (!this.db) await this.initDB();
  ...
  async cleanup(olderThanDays = 30): Promise<void> {
    ...
    const searchTransaction = this.db!.transaction(['searchResults'], 'readwrite');
    ...
    const interactionsTransaction = this.db!.transaction(['userInteractions'], 'readwrite');
    const interactionsStore = interactionsTransaction.objectStore('userInteractions');
