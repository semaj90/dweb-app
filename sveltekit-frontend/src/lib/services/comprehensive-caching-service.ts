...
class ComprehensiveCachingService {
  ...
  private getInitialStats(): CacheStats {
    ...
    return {
      ...
      overall: {
        ...
        hitRate: 0,
        missRate: 0,
        layerDistribution: layers.reduce((acc, layer) => {
          acc[layer] = 0;
          return acc;
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      console.error('❌ Failed to initialize Comprehensive Caching Service:', error);
      throw error;
    }
  }
        ...
        autoloadCallback: () => {
          ...
          if (!this.lokiCollection) {
            this.lokiCollection = this.lokiDB!.addCollection('cache', {
              indices: ['key', 'metadata.createdAt', 'metadata.lastAccessed'],
              ttl: {
                ttl: this.config.defaultTTL,
  ...
  public async clearByTags(tags: string[]): Promise<number> {
    ...
    // Clear from LokiJS
    if (this.lokiCollection) {
      const toRemove = this.lokiCollection.find({
        'metadata.tags': { '$in': tags }
      });
      clearedCount += toRemove.length;
      this.lokiCollection.removeWhere({
        'metadata.tags': { '$in': tags }
      });
    }
   ...
   */
  public async warmup(data: Array<{ key: string; value: any; options?: any }>): Promise<void> {
  ...
  private async getFromLayer<T>(key: string, layer: CacheLayer): Promise<CacheEntry<T> | null> {
    switch (layer) {
      ...
      case 'indexeddb':
        ...
        return await get(key) as CacheEntry<T> || null;
  ...
  private updateStats(): void {
    ...
    const totalEntries = Object.values(layerStats).reduce((sum, stats) => sum + stats.entries, 0);
    ...
    this.stats.set({
      ...
      overall: {
        totalEntries,
        totalSize: Object.values(layerStats).reduce((sum, stats) => sum + stats.size, 0),
