...
class WebAssemblyLlamaService {
  ...
  async loadModel(): Promise<boolean> {
    try {
      ...
      const success = (wasmModule.instance.exports as any).llama_load_model(
  ...
  private async generateDirect(prompt: string, options: any): Promise<WebLlamaResponse> {
    ...
    const wasmModule = this.module as WebAssemblyInstantiateResult;
    const promptPtr = (wasmModule.instance.exports as any).malloc(promptBytes.length);
    const memory = new Uint8Array((wasmModule.instance.exports as any).memory.buffer);
    memory.set(promptBytes, promptPtr);

    // Call WASM inference function
    const resultPtr = (wasmModule.instance.exports as any).llama_generate(
      promptPtr,
      promptBytes.length,
      maxTokens,
      temperature,
    ...
    const resultLength = (wasmModule.instance.exports as any).get_result_length(resultPtr);
    const resultBytes = memory.slice(resultPtr, resultPtr + resultLength);
    const resultText = new TextDecoder().decode(resultBytes);

    // Free allocated memory
    (wasmModule.instance.exports as any).free(promptPtr);
    (wasmModule.instance.exports as any).free(resultPtr);
