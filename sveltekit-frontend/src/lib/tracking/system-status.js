...
export class SystemStatusTracker {
  ...
  initializeServices() {
    ...
    // Migration tracking
    this.migrations.set('001_initial_schema', { status: 'pending', required: true });
    this.migrations.set('context_system', { status: 'pending', required: true });
  }
  ...
  async checkPostgres() {
    ...
    return { status: 'healthy', connection: 'postgresql://legal_admin@localhost:5432/legal_ai_db' };
  }
  ...
  async checkOllama() {
    return { status: 'healthy', models: ['gemma2:27b'], endpoint: 'http://localhost:11434' };
  }
