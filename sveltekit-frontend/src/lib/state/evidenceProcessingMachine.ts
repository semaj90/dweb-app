...
import type { DocumentProcessingJob, EmbeddingGenerationJob, AIAnalysisJob } from '../services/bullmqService';
...
export type EvidenceProcessingEvent =
  | { type: 'START_PROCESSING'; evidenceId: string; caseId: string; userId: string; filename: string; content: string; metadata?: Record<string, any> }
  | { type: 'RETRY' }
  | { type: 'CANCEL' }
  | { type: 'FORCE_COMPLETE' }
...
const documentProcessingService = fromPromise(async ({ input }: { input: EvidenceProcessingContext }) => {
  ...
  while (waited < maxWait) {
    ...
    if (jobStatus?.failedReason) {
      throw new Error(`Document processing failed: ${jobStatus.failedReason}`);
    }
...
const aiAnalysisService = fromPromise(async ({ input }: { input: EvidenceProcessingContext }) => {
  ...
  try {
    ...
    console.warn('LangChain analysis failed, using basic analysis only:', error);
  }
...
const cacheResultsService = fromPromise(async ({ input }: { input: EvidenceProcessingContext }) => {
  ...
  await Promise.all([
    multiLayerCache.set(`evidence:complete:${input.evidenceId}`, finalResult, {
      type: 'document',
      userId: input.userId,
      persistent: true,
      ttl: 604800 // 7 days
    }),
    multiLayerCache.set(`case:evidence:${input.caseId}:${input.evidenceId}`, finalResult, {
      type: 'document',
      userId: input.userId,
      persistent: true,
      ttl: 604800
...
export const evidenceProcessingMachine = createMachine({
  ...
  states: {
    ...
    documentProcessing: {
      invoke: {
        ...
        onDone: {
          ...
          actions: assign({
            extractedText: ({ event, context }) => event.output.result?.extractedText || context.content,
      ...
      invoke: {
        ...
        onDone: {
          ...
          actions: assign({
            ...
            embeddings: ({ event }) => event.output.chunks?.map((chunk: any) => chunk.embedding) || [],
            processingTimes: ({ context }) => ({
              ...context.processingTimes,
    ...
    failed: {
      type: 'final',
      entry: ({ context }) => {
        console.error(`Evidence processing failed after ${context.retryCount} retries: ${context.error}`);
      }
    },
...
export const createEvidenceProcessingActor = (context?: Partial<EvidenceProcessingContext>) => {
...
export const getProcessingTimes = (state: EvidenceProcessingState): Record<string, number> => {
