...
interface AIAgentState {
  ...
  // Chat & Conversation
  currentConversation: API.ChatMessage[];
  conversationHistory: API.ChatMessage[][];
...
const createAIAgentStore = () => {
  ...
  return {
    ...
    async sendMessage(message: string, context?: any) {
      ...
      try {
        ...
        update(state => ({
          ...
          averageResponseTime: ((state.averageResponseTime * state.totalRequests) + responseTime) / (state.totalRequests + 1),
        ...
        update(state => ({
          ...
          successRate: (state.totalRequests > 0) ? 
            (state.totalRequests * state.successRate / 100 - 1) / state.totalRequests * 100 : 0
        }));
    ...
    async handleStreamingResponse(stream: ReadableStream, jobId: string) {
      ...
      update(state => ({ ...state, isStreaming: true, streamingResponse: '' }));
      ...
      try {
        ...
        reader.releaseLock();
        update(state => ({ ...state, isStreaming: false, streamingResponse: '' }));
      }
    },
    ...
    async indexDocument(document: { title: string; content: string; metadata?: any }) {
      try {
        ...
        update(state => ({
          ...
          vectorStore: {
            ...state.vectorStore,
            documentCount: result.totalDocuments || state.vectorStore.documentCount + 1,
    ...
    saveConversation() {
      update(state => ({
        ...state,
        conversationHistory: [...state.conversationHistory, [...state.currentConversation]],
        currentConversation: [],
        activeSessionId: null
      }));
    },
    ...
    addError(error: Omit<AIError, 'id' | 'timestamp' | 'resolved'>) {
      ...
      update(state => ({
        ...
        systemHealth: state.systemHealth === 'healthy' ? 'degraded' : state.systemHealth
      }));
    },
...
export const aiErrors = derived(aiAgentStore, state => state.errors.filter(e => !e.resolved));
