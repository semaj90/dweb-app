...
import type { AIProcessingContext, AIProcessingEvents, AITask, AITaskResult } from './types';
...
export const aiProcessingMachine = createMachine({
  ...
  actions: {
    logSuccess: ({ context }) => {
      console.log(`✅ AI task ${context.task.id} completed successfully in ${Date.now() - context.timestamp}ms`);
    },
...
export const aiTaskCreators = {
  parseJSON: (data: any, options?: any) => createAITask('parse', {
    data,
    format: 'json',
    options
  }, { priority: 'high' }),
  
  trainSOM: (vectors: number[][], labels: string[], options?: any) => createAITask('som-train', {
    vectors,
    labels,
    ...options
  }, { priority: 'low', estimatedDuration: 30000 }),
  
  cudaInference: (model: string, input: any, options?: any) => createAITask('cuda-infer', {
    model,
    input,
    ...options
  }, { priority: 'high' }),
  
  generateEmbedding: (text: string, model?: string) => createAITask('embed', {
    text,
    model: model || 'nomic-embed-text'
  }, { priority: 'medium' }),
  
  analyzeDocument: (prompt: string, model?: string, format?: string) => createAITask('analyze', {
    prompt,
