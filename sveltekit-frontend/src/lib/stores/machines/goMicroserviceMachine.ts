            ...
            invoke: {
              ...
              src: fromPromise(
                async ({
                  ...
                  input: { request: GoServiceRequest; endpoint: string };
                }) => {
  ...
  },
  {
    actions: {
      ...
      logConnectionError: ({ context }) => {
        console.error(
          `❌ Failed to connect to Go microservice: ${context.error}`
        );
      },
...
export const createGoMicroserviceActor = (
  endpoint = "http://localhost:8080"
) => {
...
export const isServiceReady = (state: any) => {
  return (
    ...
    state.context.healthCheck.status === "healthy"
  );
};
