<script lang="ts">
  ...
  function handleSort(column: GridColumn) {
    ...
    if (sortConfig?.column === column.key) {
      ...
      sortConfig = { column: column.key, direction: 'asc' };
    }
  }
  ...
  function getColumnWidth(column: GridColumn): string {
    const customWidth = columnWidths.get(column.key);
  ...
  function formatCellValue(value: any, column: GridColumn, row: any): string {
    if (column.formatter) {
          ...
          >
            ...
            {#each columns as column}
              <div
                ...
                style:width={getColumnWidth(column)}
                on:onclick={() => startEdit(rowIndex, column.key)}
              >
...
<style>
  .yorha-data-grid {
    @apply bg-black border-2 border-amber-400 relative overflow-hidden font-mono;
    box-shadow: 0 0 30px rgba(255, 191, 0, 0.4);
  }

  .yorha-data-grid::before {
    content: '';
    position: absolute;
    top: 0;
    left: 0;
  ...
  .yorha-glitch-effect {
    animation: grid-glitch 0.4s infinite;
  }
  ...
  .yorha-grid-header {
    @apply flex items-center justify-between p-4 bg-amber-400 text-black font-bold;
  ...
  .yorha-grid-filters {
    @apply flex flex-wrap gap-4 p-3 bg-gray-900 border-b border-amber-400;
  }
  ...
  .yorha-filter-label {
    @apply text-xs text-amber-400 font-bold uppercase;
  }

  .yorha-filter-input {
    @apply bg-black border border-amber-400 text-amber-400 px-2 py-1 text-xs;
  ...
  .yorha-spinner-ring {
    @apply w-12 h-12 border-4 border-amber-400 border-t-transparent rounded-full;
    animation: spin 1s linear infinite;
  }

  .yorha-loading-text {
    @apply text-amber-400 font-bold tracking-wide;
  }

  .yorha-grid-header-row {
    @apply flex sticky top-0 z-20 bg-amber-400 text-black border-b-2 border-amber-600;
    background: linear-gradient(45deg, #ffbf00, #ffd700);
  }
  ...
  .yorha-grid-row {
    @apply flex absolute w-full border-b border-amber-400 border-opacity-20;
    @apply hover:bg-amber-400 hover:bg-opacity-10 transition-colors;
  }

  .yorha-row-selected {
    @apply bg-amber-400 bg-opacity-20;
  }
  ...
  .yorha-grid-cell {
    @apply flex items-center px-3 py-2 border-r border-amber-400 border-opacity-20;
    @apply text-amber-400 text-sm;
    min-height: 40px;
  }

  .yorha-header-cell {
    @apply font-bold text-black bg-transparent cursor-pointer;
  ...
  .yorha-editable:hover {
    @apply bg-amber-400 bg-opacity-5;
  }

  .yorha-editing {
    @apply bg-amber-400 bg-opacity-15;
  }

  .yorha-select-cell {
    @apply w-12 justify-center flex-shrink-0;
  }
  ...
  .yorha-resize-handle {
    @apply absolute right-0 top-0 bottom-0 w-1 bg-transparent cursor-col-resize;
  ...
  .yorha-edit-input {
    @apply w-full bg-black border border-amber-400 text-amber-400 px-2 py-1 text-sm;
    @apply focus:outline-none focus:border-amber-300;
  }
  ...
  .yorha-action-btn {
    @apply bg-amber-400 text-black px-2 py-1 text-xs font-bold;
    @apply hover:bg-amber-300 transition-colors;
