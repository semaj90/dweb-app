...
export function vscodeErrorLogger(options: VSCodeErrorConfig = {}): Plugin {
    ...
    try {
      ...
      const logData = {
        metadata: {
          ...
          viteVersion: server?.config?.plugins?.find(p => p.name === 'vite:build')?.name || 'unknown',
  ...
  function createDiagnosticsFile() {
    ...
    try {
      const diagnosticsData = {
        ...
        summary: {
          totalErrors: diagnostics.filter(d => d.severity === 1).length,
          totalWarnings: diagnostics.filter(d => d.severity === 2).length,
          totalInfo: diagnostics.filter(d => d.severity === 3).length,
          affectedFiles: Array.from(new Set(diagnostics.map(d => d.uri))).length
        }
      };
  ...
  return {
    ...
    buildStart() {
      ...
      errorLog = errorLog.filter(entry => entry.buildPhase !== 'build');
      diagnostics = diagnostics.filter(d => d.source !== 'vite-build');
