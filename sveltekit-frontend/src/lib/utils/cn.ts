...
export function priorityClass(priority: 'critical' | 'high' | 'medium' | 'low'): string {
	const baseClass = 'inline-flex items-center px-2 py-1 rounded-full text-xs font-medium';
	
	switch (priority) {
		case 'critical':
			return cn(baseClass, 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400');
		case 'high':
			return cn(baseClass, 'bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400');
		case 'medium':
			return cn(baseClass, 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400');
		case 'low':
			return cn(baseClass, 'bg-gray-100 text-gray-800 dark:bg-gray-900/20 dark:text-gray-400');
		default:
			return cn(baseClass, 'bg-gray-100 text-gray-800 dark:bg-gray-900/20 dark:text-gray-400');
	}
}
...
export function evidenceTypeClass(type: string): string {
	const baseClass = 'inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium';
	
	switch (type) {
		case 'document':
			return cn(baseClass, 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400');
		case 'image':
			return cn(baseClass, 'bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-400');
		case 'video':
			return cn(baseClass, 'bg-pink-100 text-pink-800 dark:bg-pink-900/20 dark:text-pink-400');
		case 'audio':
			return cn(baseClass, 'bg-indigo-100 text-indigo-800 dark:bg-indigo-900/20 dark:text-indigo-400');
		case 'transcript':
			return cn(baseClass, 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400');
		default:
			return cn(baseClass, 'bg-gray-100 text-gray-800 dark:bg-gray-900/20 dark:text-gray-400');
	}
}
...
export function aiStatusClass(status: 'online' | 'processing' | 'offline' | 'warning'): string {
...
// NieR theme variants
export function nierVariant(variant: 'primary' | 'secondary' | 'accent' | 'gothic'): string {
	switch (variant) {
		case 'primary':
...
export function componentState(state: 'default' | 'hover' | 'active' | 'disabled' | 'loading'): string {
	switch (state) {
		case 'hover':
...
export function responsiveGrid(items: number): string {
	...
	if (items <= 6) return 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4';
...
export function workflowState(state: 'draft' | 'review' | 'approved' | 'rejected' | 'archived'): string {
