...
export class QdrantService {
    ...
    constructor() {
        ...
        logger.info('QdrantService initialized', { url: qdrantUrl, vectorSize: this.VECTOR_SIZE });
    }
            ...
            for (const [key, collectionName] of Object.entries(this.COLLECTIONS)) {
                ...
                if (!exists) {
                    ...
                    if (info.config?.params?.vectors?.size !== this.VECTOR_SIZE) {
                        logger.error(`Collection ${collectionName} has wrong dimensions: ${info.config?.params?.vectors?.size}`);
    ...
    async batchStoreDocuments(documents: DocumentVector[]): Promise<BatchUpsertResult> {
        try {
            ...
            const invalidDocs = documents.filter(d => d.vector.length !== this.VECTOR_SIZE);
            ...
            return {
                ...
                failed: 0,
                ids: points.map(p => p.id.toString())
            };
        } catch (error) {
    ...
    async searchSimilar(
        ...
        try {
            ...
            if (filter.case_id) {
                qdrantFilter.must = [{ key: 'case_id', match: { value: filter.case_id } }];
            }
            if (filter.type) {
            ...
            return results.map(result => ({
                ...
                relevance_score: result.payload?.relevance_score || result.score,
    ...
    async getDocument(id: string, collection: string = this.DEFAULT_COLLECTION): Promise<DocumentVector | null> {
        try {
            ...
            return {
                ...
                relevance_score: point.payload?.relevance_score
            };
        } catch (error) {
     ...
     */
    async getCollectionStats(collection: string = this.DEFAULT_COLLECTION): Promise<any> {
        try {
