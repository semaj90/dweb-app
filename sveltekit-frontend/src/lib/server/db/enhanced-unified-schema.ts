// Enhanced Unified Schema Aggregator
// Combines authentication/session tables, evidence & analysis domain tables, and auxiliary entities.
// Source of truth for Drizzle migrations (referenced by drizzle.config.ts)
// Keeping imports explicit to ensure tree-shaking does not drop table definitions.

// Auth + core domain (snake_case columns aligned to existing DB) ---------------------------------
export * from './schema-postgres';

// Evidence & analysis domain (higher-level, camelCase / mixed naming) ----------------------------
// NOTE: These tables live in ../schema.ts. We re-export under their original names.
export {
  ...
  evidenceTable as evidence_v2, // alias to avoid name collision with schema-postgres `evidence`
...
// IMPORTANT:
// We deliberately alias overlapping conceptual tables (e.g. evidence, cases, reports) to avoid
// Drizzle duplicate identifier collisions. Migrations will only manage the auth + snake_case
// variants as authoritative. The suffixed *_v2 tables remain available for legacy / transitional
// code paths and can be incrementally merged later.

// TODO (follow-up):
// 1. Map/merge data from v2 tables into unified snake_case equivalents where appropriate.
// 2. Remove legacy aliases once all routes/services standardized on the snake_case versions.
/**
