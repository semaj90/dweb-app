...
import { pgTable, text, timestamp, uuid, boolean, integer, real, jsonb, vector, index } from 'drizzle-orm/pg-core';
...
export const cases = pgTable('cases', {
  ...
  titleEmbeddingIdx: index('cases_title_embedding_idx').using('ivfflat', table.titleEmbedding),
  contentEmbeddingIdx: index('cases_content_embedding_idx').using('ivfflat', table.contentEmbedding),
}));
...
export const evidence = pgTable('evidence', {
  ...
  titleEmbeddingIdx: index('evidence_title_embedding_idx').using('ivfflat', table.titleEmbedding),
  contentEmbeddingIdx: index('evidence_content_embedding_idx').using('ivfflat', table.contentEmbedding),
}));
...
export const criminals = pgTable('criminals', {
  ...
  profileEmbeddingIdx: index('criminals_profile_embedding_idx').using('ivfflat', table.profileEmbedding),
}));
...
export const evidenceConnections = pgTable('evidence_connections', {
  ...
  fromEvidenceIdx: index('evidence_connections_from_idx').on(table.fromEvidenceId),
...
export const vectorMetadata = pgTable('vector_metadata', {
  ...
  documentIdIdx: index('vector_metadata_document_id_idx').on(table.documentId),
  documentTypeIdx: index('vector_metadata_document_type_idx').on(table.documentType),
  collectionIdx: index('vector_metadata_collection_idx').on(table.collectionName),
}));
...
export const embeddingCache = pgTable('embedding_cache', {
  ...
  embeddingIdx: index('embedding_cache_embedding_idx').using('ivfflat', table.embedding),
}));
...
export const userActivity = pgTable('user_activity', {
  ...
  resourceTypeIdx: index('user_activity_resource_type_idx').on(table.resourceType),
