...
import { embedding_cache, vector_metadata, cases, evidence, criminals } from '../db/schema-postgres.js';
...
export class VectorService {
    ...
    try {
      ...
      try {
        const cached = await db
          ...
          .where(eq(embedding_cache.textHash, Buffer.from(text).toString('base64')))
          .limit(1);
  ...
  async storeDocument(
    ...
    try {
      ...
      await db.insert(vector_metadata).values({
        id: cuid2.createId(),
        documentId: id,
        ...
        target: vector_metadata.documentId,
        set: {
          metadata: metadata,
          contentHash: Buffer.from(content).toString('base64'),
          updatedAt: new Date()
  ...
  async search(
    ...
    try {
      ...
      // Format results
      const results: EmbeddingResult[] = searchResult.map(point => ({
  ...
  async hybridSearch(
    ...
    try {
      // Perform vector search
      const vectorResults = await this.search(query, { ...options, limit: limit * 2 });
  ...
  private async keywordSearch(
    ...
    try {
      ...
      if (!filter.type || filter.type === 'case') {
        ...
        results.push(...caseResults.map(c => ({
          id: c.id,
      ...
      if (!filter.type || filter.type === 'evidence') {
        ...
        results.push(...evidenceResults.map(e => ({
          id: e.id,
          score: 0.8,
      ...
      if (!filter.type || filter.type === 'criminal') {
        ...
        results.push(...criminalResults.map(c => ({
          id: c.id,
          score: 0.8,
  ...
  async findSimilar(
    ...
    try {
      ...
      const results = similar
        .filter(p => p.id.toString() !== documentId)
        .map(point => ({
          id: point.id.toString(),
          score: point.score,
  ...
  async bulkIndex(
    ...
    try {
      ...
      for (let i = 0; i < documents.length; i += batchSize) {
        ...
        await db.insert(vector_metadata).values(metadataRecords).onConflictDoNothing();
  ...
  async deleteDocument(documentId: string): Promise<void> {
    try {
      ...
      await db.delete(vector_metadata).where(eq(vector_metadata.documentId, documentId));
