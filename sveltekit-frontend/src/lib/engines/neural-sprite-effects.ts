...
export class NeuralSpriteEffects {
  ...
  async applyMatrixRain(speed: number = 50, color: string = '#00ff41'): Promise<void> {
    const canvasWidth = this.canvas.getWidth();
    const canvasHeight = this.canvas.getHeight();
    ...
    this.activeEffects.set('matrix-rain', { interval: rainInterval, drops: rainDrops });
  }
  ...
  async applyGlitchEffect(): Promise<void> {
    ...
    objects.forEach(obj => {
      if (Math.random() < 0.3) { // 30% chance to glitch each object
  ...
  async applyNeuralNetworkEffect(): Promise<void> {
    ...
    for (let i = 0; i < nodes.length; i++) {
      for (let j = i + 1; j < nodes.length; j++) {
        ...
        if (distance < 150) { // Connect nearby nodes
          const connection = new fabric.Line([
            node1.left, node1.top,
            node2.left, node2.top
          ], {
            stroke: '#00aaff',
            strokeWidth: 2,
            opacity: 0.3,
  ...
  async applyLegalHighlightEffect(evidenceObjects: fabric.Object[]): Promise<void> {
  ...
  clearEffect(effectId: string): void {
    ...
    if (effect) {
      ...
      ['drops', 'nodes', 'connections', 'objects', 'particles'].forEach(prop => {
        if (effect[prop]) {
...
export const NEURAL_SPRITE_EFFECTS: SpriteEffect[] = [
  ...
  },
  {
    ...
    apply: async (canvas, options) => {
      const effects = new NeuralSpriteEffects(canvas);
      await effects.applyMatrixRain(options?.speed || 50, options?.color || '#00ff41');
    }
  },
  {
    id: 'neural-network',
