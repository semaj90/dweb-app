...
export class BrowserCacheManager {
  ...
  private handleServiceWorkerMessage(event: MessageEvent): void {
    ...
    switch (type) {
      case 'SPRITE_CACHED':
        ...
        break;
      case 'CACHE_FULL':
        console.warn('Service Worker sprite cache is full, evicting old sprites');
        break;
    }
  }
  ...
  public async cacheSprite(sprite: any): Promise<void> {
    ...
    const compressed = this.config.enableCompression ? 
      await this.compressData(spriteData) : spriteData;
  ...
  public getCacheStats(): {
    ...
    const totalAccess = cacheValues.reduce((sum, sprite) => sum + sprite.accessCount, 0);
