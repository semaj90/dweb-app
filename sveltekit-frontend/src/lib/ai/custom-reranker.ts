...
export async function enhancedSearchWithNeo4j(
  ...
  const { searchDocuments } = await import('./qdrant-service');
  
  const annResults = await searchDocuments(query, limit * 2); // Get more for reranking
  
  // Convert to rerank format with Neo4j enrichment
  const rerankInput: RerankResult[] = annResults.map(result => ({
    id: result.id,
    ...
    metadata: {
      ...
      userFrequency: neo4jContext ? calculateFrequencyScore(result, neo4jContext) : 0
    },
 ...
 */
function calculatePathScore(result: any, neo4jContext: Neo4jPathContext): number {
  let pathScore = 0;
 ...
 */
function calculateFrequencyScore(result: any, neo4jContext: Neo4jPathContext): number {
