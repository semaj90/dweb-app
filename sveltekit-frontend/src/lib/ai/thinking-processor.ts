...
export class ThinkingProcessor {
  ...
  static parseThinkingResponse(
    ...
    return {
      thinking,
      analysis: this.extractJSON(afterThinking) || {
  ...
  private static calculateConfidence(
    ...
    if (thinking.includes("evidence") || thinking.includes("analysis"))
      score += 0.1;
    if (thinking.includes("legal") || thinking.includes("compliance"))
      score += 0.1;

    // Boost for structured analysis
    if (analysis.includes("confidence") || analysis.includes("recommendations"))
      score += 0.1;
    if (
      analysis.includes("key_findings") ||
      analysis.includes("legal_implications")
    )
      score += 0.1;
