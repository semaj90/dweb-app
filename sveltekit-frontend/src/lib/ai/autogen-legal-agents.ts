...
Ensure all recommendations comply with legal and ethical standards.`,
  ...
  async analyzeCase(request: LegalAnalysisRequest): Promise<LegalAnalysisResult> {
    ...
    try {
      ...
      const taskBreakdown = await this.queryAgent('chief_analyst', 
...
4. Analysis coordination strategy`, conversations);
      ...
      const strategyAnalysis = await this.queryAgent('strategy_advisor',
...
Provide specific strategic recommendations and identify potential challenges.`, conversations);

      // Phase 4: Chief Analyst synthesizes final analysis
      const finalAnalysis = await this.queryAgent('chief_analyst',
...
5. Overall confidence level (0.0-1.0)`, conversations);
  ...
  private async runEvidenceAnalysis(request: LegalAnalysisRequest, conversations: ConversationMessage[]): Promise<string> {
    return await this.queryAgent('evidence_specialist', 
...
Provide specific recommendations for evidence handling and presentation.`, conversations);
  }

  private async runLegalResearch(request: LegalAnalysisRequest, conversations: ConversationMessage[]): Promise<string> {
    return await this.queryAgent('research_specialist',
...
Provide citations and relevance analysis for each source.`, conversations);
  }

  private async runComplianceReview(request: LegalAnalysisRequest, conversations: ConversationMessage[]): Promise<string> {
    return await this.queryAgent('compliance_officer',
...
Identify any compliance issues and provide mitigation strategies.`, conversations);
  }

  private async queryAgent(agentName: string, prompt: string, conversations: ConversationMessage[]): Promise<string> {
  ...
  private async queryOllama(agent: AgentConfig, prompt: string): Promise<string> {
...
export { AutogenLegalTeam, type LegalAnalysisRequest, type LegalAnalysisResult, type AgentConfig };