...
export class MatrixUICompiler {
   ...
   */
  async compileEnhanced(nodes: MatrixUINode[], xstateContext?: any): Promise<{
    ...
    const cssOutput = await this.generateEnhancedCSS(optimizedNodes, xstateContext);
   ...
   * Create WebGL buffer for GPU acceleration
   */
  private createWebGLBuffer(node: MatrixUINode, matrix: mat4): WebGLBuffer | undefined {
