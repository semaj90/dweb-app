...
<div class="gallery-page">
    ...
    <div class="header-content">
      ...
      <div class="header-actions">
        <button 
          ...
          on:on:onclick={() => showUploadModal = true}
        >
          📤 Upload Files
        </button>
        <button 
          class="nes-btn is-normal"
          on:on:onclick={() => loadGalleryData()}
          disabled={isLoading}
        >
          {isLoading ? '🔄' : '↻'} Refresh
        </button>
    ...
    <div class="controls-grid">
      ...
      <div class="control-group">
        <button 
          ...
          on:on:onclick={() => sortOrder = sortOrder === 'desc' ? 'asc' : 'desc'}
        >
          {sortOrder === 'desc' ? '↓' : '↑'}
        </button>
      </div>
      ...
      <div class="control-group view-modes">
        ...
        <button 
          ...
          on:on:onclick={() => viewMode = 'masonry'}
        >
          ⊡
        </button>
      </div>
  ...
  {#if error}
    <div class="error-state nes-container is-error">
      <p>❌ {error}</p>
      <button class="nes-btn is-normal" on:on:onclick={() => loadGalleryData()}>
        Retry
      </button>
    </div>
  {/if}
      ...
      <div class="gallery-grid gallery-{viewMode}">
        {#each filteredItems as item}
          <div class="gallery-item nes-container is-rounded">
            <div class="item-preview" on:on:onclick={() => openItem(item)}>
              ...
              <div class="item-overlay">
                ...
                <div class="overlay-actions">
                  <button class="nes-btn is-small" on:on:onclick={(e) => { e.stopPropagation(); downloadItem(item); }}>
                    ⬇️
                  </button>
                  <button class="nes-btn is-small" on:on:onclick={(e) => { e.stopPropagation(); shareItem(item); }}>
                    📤
                  </button>
                </div>
              </div>
  ...
  <!-- Upload Modal -->
  {#if showUploadModal}
    <div class="modal-overlay" on:on:onclick={() => showUploadModal = false}>
      <div class="modal-content nes-container is-rounded" on:on:onclick={(e) => e.stopPropagation()}>
        <div class="modal-header">
          <h3>Upload Files</h3>
          <button class="nes-btn is-error" on:on:onclick={() => showUploadModal = false}>×</button>
        </div>
  ...
  <!-- Item Detail Modal -->
  {#if selectedItem}
    <div class="modal-overlay" on:on:onclick={closeModal}>
      <div class="modal-content detail-modal nes-container is-rounded" on:on:onclick={(e) => e.stopPropagation()}>
        <div class="modal-header">
          <h3>{selectedItem.title || 'Gallery Item'}</h3>
          <button class="nes-btn is-error" on:on:onclick={closeModal}>×</button>
        </div>
        <div class="modal-body">
          ...
          <div class="detail-actions">
            <button class="nes-btn is-success" on:on:onclick={() => downloadItem(selectedItem)}>
              ⬇️ Download
            </button>
            <button class="nes-btn is-primary" on:on:onclick={() => shareItem(selectedItem)}>
              📤 Share
            </button>
            ...
            {/if}
            <button class="nes-btn is-error" on:on:onclick={() => deleteItem(selectedItem)}>
              🗑️ Delete
            </button>
          </div>
        </div>
