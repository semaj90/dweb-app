...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    let documentMetadata: Array<{ id: string; type: string; keywords: string[] }> = [];
    ...
    try {
      ...
      try {
        ...
        console.warn('Qdrant retrieval failed, using PostgreSQL only:', qdrantError);
      }
      ...
      embeddings = Array.from(mergedDocuments.values()).map(doc => doc.embedding);
    ...
    try {
      ...
      const analysis = await kmeans.analyzeLegalClusters(embeddings, documentMetadata);
