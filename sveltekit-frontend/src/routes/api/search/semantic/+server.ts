...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    const { query, clusterId, useKnowledge = true, limit = 20, threshold = 0.7 } = await request.json();
    ...
    try {
      const pgResults = await db
        .select({
          ...
          similarity: sql<number>`1 - (${documents.embeddings} <=> ${JSON.stringify(queryEmbedding)})`
        })
        .from(documents)
        .where(
          and(
            sql`${documents.embeddings} <=> ${JSON.stringify(queryEmbedding)} < ${1 - threshold}`,
            clusterId ? eq(documents.metadata, sql`jsonb_build_object('clusterId', ${clusterId})`) : sql`true`
          )
        )
        .orderBy(desc(sql`1 - (${documents.embeddings} <=> ${JSON.stringify(queryEmbedding)})`))
        .limit(limit);
    ...
    try {
      ...
      for (const result of qdrantResults.result || []) {
    ...
    if (useKnowledge) {
      try {
        ...
        if (som) {
          ...
          clusterInfo.push(`SOM cluster: (${queryCluster.x}, ${queryCluster.y}) confidence: ${queryCluster.confidence.toFixed(3)}`);
    ...
    const clusterInsights = await getClusterInsights(queryEmbedding, clusterInfo);
 ...
 */
async function enhanceLegalContext(results: DocumentCluster[], query: string): Promise<DocumentCluster[]> {
  const legalKeywords = [
    'contract', 'liability', 'compliance', 'regulation', 'statute',
    'precedent', 'jurisdiction', 'defendant', 'plaintiff', 'evidence',
    'testimony', 'warrant', 'subpoena', 'discovery', 'motion'
  ];
  
  const queryWords = query.toLowerCase().split(/\\s+/);
  const legalScore = queryWords.filter(word => 
    legalKeywords.some(keyword => word.includes(keyword))
  ).length / queryWords.length;
  
  return results.map(result => {
    const text = (result.metadata.extractedText || '').toLowerCase();
    const textLegalScore = legalKeywords.filter(keyword => 
      text.includes(keyword)
    ).length / legalKeywords.length;
 ...
 */
async function getClusterInsights(queryEmbedding: number[], clusterInfo: string[]): Promise<string[]> {
  const insights: string[] = [];
  
  try {
    // Get related legal topics from clustering
    const cachedInsights = await redis.get(`insights:${JSON.stringify(queryEmbedding).slice(0, 50)}`);
    if (cachedInsights) {
...
export const GET: RequestHandler = async () => {
  try {
    ...
    clusterStatus.queue.pending = queueKeys.filter(key => key.includes('queued')).length;
    clusterStatus.queue.processing = queueKeys.filter(key => key.includes('processing')).length;
