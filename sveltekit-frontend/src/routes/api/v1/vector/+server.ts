...
async function determineProcessingPath(
	...
	const preferences = request.options || {};
...
async function getHealthStatus() {
	try {
		...
		if (serviceValues.every(s => s === 'connected')) {
			...
			health.overall = 'unhealthy' as const;
		}
...
export const GET_STATUS: RequestHandler = async ({ params, url }) => {
	const jobId = params.jobId || url.searchParams.get('jobId');
	...
	try {
		...
		const job = await db.default
			.select()
			.from(vectorJobs)
			.where(eq(vectorJobs.id, jobId))
			.limit(1);
