...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    ...
    const precedentsQuery = db
      ...
      .limit(limit)
      .offset(offset)
      .orderBy(desc(legalPrecedents.relevanceScore), desc(legalPrecedents.year));
    ...
    return json({ error: "Failed to search legal precedents" }, { status: 500 });
  }
};
...
export const PUT: RequestHandler = async ({ request }) => {
  try {
    ...
    const legalPrinciples = basePrecedent[0].legalPrinciples as string[] || [];
      ...
      .where(
        and(
          ...
          legalPrinciples.length > 0 
            ? or(...legalPrinciples.map(principle => 
    ...
    return json({ error: "Failed to find similar precedents" }, { status: 500 });
  }
};