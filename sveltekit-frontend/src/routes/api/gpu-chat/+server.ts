...
const WS_PORT_OFFSET = 2; // WebSocket on 5175
let currentPort = PRIMARY_PORT;
let wsPort = PRIMARY_PORT + WS_PORT_OFFSET;
...
const SERVICES = {
  ...
  grpc: 'localhost:50051' // gRPC with TLS
};
...
const GPU_CONFIG = {
  ...
  tensorRT: {
    enabled: true,
    precision: 'FP16', // FP16 for RTX 3060
...
async function handleWebSocketMessage(clientId: string, message: any) {
  ...
  switch (message.type) {
    case 'chat':
      const response = await processWithGPU(message.content);
    ...
    case 'batch':
      const results = await processBatch(message.items);
      client.ws.send(JSON.stringify({
        type: 'batch_complete',
        results
      }));
...
async function processBatch(items: any[]): Promise<any[]> {
  try {
    const response = await fetch(`${SERVICES.goMicroservice}/api/batch`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        items,
        parallel: true,
        gpuBatch: GPU_CONFIG.tensorRT.batchSize
      })
    });
    ...
    return items.map(item => ({ ...item, error: 'Processing failed' }));
  }
}
