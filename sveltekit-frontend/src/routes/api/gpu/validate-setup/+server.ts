...
export const GET: RequestHandler = async () => {
  ...
  const now = Date.now();
  type AISummarizeChecks = { gpu: boolean; ollama: boolean; model: boolean };
  type Details = {
    ollama: {
      ok: boolean;
      models_count: number;
  ...
  type CachePayload = { ok: boolean; details: Details; message?: string };
  const cache: { ts?: number; data?: CachePayload } =
    (
      globalThis as unknown as {
        __gpu_validate_cache?: { ts?: number; data?: CachePayload };
      }
    ).__gpu_validate_cache || {};
  ...
  type GPUStatus = {
    available?: boolean;
    cuda?: { available?: boolean; version?: string; device_id?: number };
    device?: { name?: string };
    memory?: { free?: number; total?: number; used?: number };
    temperature_c?: number;
  };
  type HealthPayload = { services?: { gpu?: string | boolean } };
  ...
  if (gpuStatus.status === "rejected") {
    ...
    gpuStatus = (
      await Promise.allSettled([fetchWithTimeout<GPUStatus>("/api/gpu-status")])
    )[0];
  }
  ...
  const ollama_ok =
    ollamaHealthy.status === "fulfilled" && Boolean(ollamaHealthy.value);
  ...
  const ai_ready =
    details.ai_summarize_checks.gpu &&
    details.ai_summarize_checks.ollama &&
    details.ai_summarize_checks.model;
  const message = !model_present
    ? "please downloal local llm and put in this directory"
    : undefined;

  const payload: CachePayload = { ok: ai_ready, details, message };
  (
    globalThis as unknown as {
      __gpu_validate_cache?: { ts?: number; data?: CachePayload };
    }
  ).__gpu_validate_cache = { ts: now, data: payload };
  return json(payload);
};
