...
`;
  ...
  try {
    await writeFile(join(__dirname, 'src/lib/types/user.ts'), userTypeFix, 'utf8');
  ...
  try {
    ...
    for (let i = 200; i < Math.min(205, lines.length); i++) {
      if (lines[i] && lines[i].trim() === '') {
        ...
        if (lines[i-1] && lines[i-1].includes('/>') && !lines[i-1].includes('</div>')) {
          lines[i] = '          </div>';
...
async function fixIndexExports() {
  ...
  try {
    ...
    const buttonIndexPath = join(__dirname, 'src/lib/components/ui/button/index.ts');
...
async function fixDatabaseSchemaErrors() {
  ...
  try {
    ...
    if (content.includes('export const personsOfInterest') && !content.includes('profileImageUrl')) {
      content = content.replace(
  ...
  caseId: uuid("case_id").references(() => cases.id),`
    ...
    if (content.includes('export const evidenceVectors') && !content.includes('caseId')) {
      content = content.replace(
...
`;
    ...
    await writeFile(join(__dirname, 'src/lib/types/canvas.ts'), canvasTypesContent, 'utf8');
...
async function main() {
  try {
    ...
    console.log(`   📦 Package.json duplicates: ${fixes.packageJsonDuplicates}`);
    ...
    console.log(`   ⚠️ CSS warnings: ${fixes.cssWarnings} (mostly unused selectors - safe to ignore)`);
