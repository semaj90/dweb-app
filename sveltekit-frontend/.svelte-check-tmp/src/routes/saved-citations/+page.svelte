            ...
            <DropdownMenuRoot>
              {#snippet children({ trigger, menu })}
                ...
                <DropdownMenuContent {menu}>
                  <DropdownMenuItem click={() => toggleFavorite(citation)}>
                  ...
                  <DropdownMenuItem click={() => editCitation(citation)}>
...
<DialogRoot open={showAddDialog} openchange={(open) => showAddDialog = open}>
  <DialogContent class="sm:max-w-[425px]" overlay={{}} content={{}} openState={showAddDialog}>
    ...
    <DialogFooter>
      <Button variant="secondary" click={() => (showAddDialog = false)}>Cancel</Button>
      <Button click={() => saveCitation()} disabled={!newCitation.title || !newCitation.content}>
        Save Citation
      </Button>
    </DialogFooter>
  </DialogContent>
...
{#if editingCitation}
  <DialogRoot open={true} openchange={() => (editingCitation = null)}>
    <DialogContent class="sm:max-w-[425px]" overlay={{}} content={{}} openState={true}>
      ...
      <DialogFooter>
        <Button variant="secondary" click={() => (editingCitation = null)}>Cancel</Button>
        <Button click={() => updateCitation()}>Update Citation</Button>
      </DialogFooter>
    </DialogContent>
  </DialogRoot>
{/if}
