...
export const GET: RequestHandler = async ({ url }) => {
      ...
      if (dbConnected) {
        ...
        testResults.postgresql = {
          ...
          details: {
            ...
            connectionString: process.env.DATABASE_URL ? '[REDACTED]' : 'Using default fallback'
          }
        };
      } else {
        testResults.postgresql = {
    ...
    if (minioService) {
      ...
      try {
        ...
        testResults.minio = {
          ...
          details: {
            endpoint: `${process.env.MINIO_HOST || 'localhost'}:${process.env.MINIO_PORT || '9000'}`,
  ...
  testResults.environment = {
    ...
    variables: {
      ...
      DEV_DATABASE_URL: process.env.DEV_DATABASE_URL ? '✅ Set' : '❌ Missing',
      REDIS_URL: process.env.REDIS_URL ? '✅ Set' : '❌ Missing',
      MINIO_HOST: process.env.MINIO_HOST ? '✅ Set' : '❌ Missing',
      POSTGRES_USER: process.env.POSTGRES_USER ? '✅ Set' : '❌ Missing',
      POSTGRES_PASSWORD: process.env.POSTGRES_PASSWORD ? '✅ Set' : '❌ Missing',
      NODE_ENV: process.env.NODE_ENV || 'undefined'
    },
    summary: {
      database: process.env.DATABASE_URL || process.env.DEV_DATABASE_URL || 'Using fallback',
      user: process.env.POSTGRES_USER || 'postgres',
      database_name: process.env.POSTGRES_DB || 'legal_ai_db'
    }
  };
