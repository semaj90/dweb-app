...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const model = process.env.OLLAMA_MODEL ?? reqModel ?? 'gemma3:legal:latest';
    const temperature =
      reqTemperature !== undefined
      ? Number(reqTemperature)
      : Number(process.env.OLLAMA_TEMPERATURE ?? 0.7);
    const maxTokens =
      reqMaxTokens !== undefined
      ? Number(reqMaxTokens)
      : Number(process.env.OLLAMA_MAX_TOKENS ?? 2048);
    const stream =
      reqStream !== undefined
      ? Boolean(reqStream)
      : (process.env.OLLAMA_STREAM ?? 'false') === 'true';
    const useVectorSearch =
      reqUseVectorSearch !== undefined
      ? Boolean(reqUseVectorSearch)
      : (process.env.OLLAMA_USE_VECTOR_SEARCH ?? 'true') === 'true';
