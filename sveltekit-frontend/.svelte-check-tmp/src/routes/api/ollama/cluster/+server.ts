...
const CLUSTER_STATUS_CACHE_TTL = Number(process.env.CLUSTER_STATUS_CACHE_TTL ?? 5000); // ms
...
export async function getClusterStatus(detailed: boolean = false): Promise<ClusterStatus> {
  ...
  const strategy = (process.env.CLUSTER_STRATEGY as ClusterStatus['loadBalancing']['strategy']) ??
...
async function getInstanceStatus(instanceId: string, config?: any): Promise<OllamaInstance | null> {
  try {
    ...
    const MAX_RETRIES = 2;
    const TIMEOUT_MS = Number(process.env.INSTANCE_STATUS_TIMEOUT_MS ?? 3000);
