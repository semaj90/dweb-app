...
<div class="case-workspace container mx-auto px-4">
  <header class="case-header">
    ...
    <div class="ai-controls">
      <button class="btn-toggle" onclick={() => sidebarOpen = !sidebarOpen}>
        {sidebarOpen ? "◀ Hide Sidebar" : "▶ Show Sidebar"}
      </button>

      <button class="btn-ai" onclick={generateCaseSummary} disabled={aiGenerating}>
        {#if aiGenerating}
      ...
      <button class="btn-ai" onclick={generateProsecutionStrategy} disabled={aiGenerating}>
  ...
  <div class="workspace-layout ssr-layout bits-ui">
    <aside class="sidebar nes-container with-title" class:open={sidebarOpen} aria-hidden={!sidebarOpen} title="Sidebar">
      ...
      <div class="sidebar-tabs">
        <button class="tab-btn nes-btn" class:active={activeTab === 'evidence'} onclick={() => activeTab = "evidence"} title="Evidence List (Ctrl/Cmd + 3)">📁 Evidence</button>
        <button class="tab-btn nes-btn" class:active={activeTab === 'reports'} onclick={() => activeTab = "reports"} title="AI Reports (Ctrl/Cmd + 4)">📊 AI Reports</button>
        <button class="tab-btn nes-btn" class:active={activeTab === 'images'} onclick={() => activeTab = "images"} title="Image Generator (Ctrl/Cmd + 5)">🎨 Image Gen</button>
      </div>
            ...
            {#each evidenceList as evidence}
              <div class="evidence-item nes-container is-rounded" draggable={true}>
                ...
                <div class="evidence-actions">
                  <button class="add-to-canvas-btn nes-btn is-success" onclick={() => canvasEditor?.addEvidenceToCanvas(evidence)}>➕ Add</button>
                  ...
                  <details class="nes-container is-rounded" style="display:inline-block; margin-left:0.5rem;">
                    ...
                    <div style="padding:0.5rem; max-width:320px;">
                      ...
                      <div style="margin-top:0.5rem;">
                        <button class="nes-btn is-primary" onclick={() => canvasEditor?.addEvidenceToCanvas(evidence)}>Add to Canvas</button>
            ...
            {#each aiReports as report}
              <div class="report-item nes-container is-rounded">
                ...
                <div style="display:flex; gap:0.5rem; margin-top:0.5rem;">
                  <button class="btn-small nes-btn is-primary" onclick={() => reportEditor?.setContent(report.richTextContent)}>Load into Editor</button>
    ...
    <main class="main-content">
      ...
      <div class="content-tabs nes-container">
        <div>
          <button class="tab-btn nes-btn" class:active={activeTab === "canvas"} onclick={() => activeTab = "canvas"} title="Interactive Canvas (Ctrl/Cmd + 1)">🎨 Canvas</button>
          <button class="tab-btn nes-btn" class:active={activeTab === "editor"} onclick={() => activeTab = "editor"} title="Report Editor (Ctrl/Cmd + 2)">📝 Editor</button>
        </div>
