    ...
    >
      {#if selectedFiles.length === 0}
        ...
        <input
          ...
          id="file-input"
        />
        <Button variant="outline" click={() => document.getElementById('file-input')?.click()}>
          Select Files
        </Button>
      {:else}
        <div class="space-y-3">
          ...
          <div class="flex justify-center space-x-3 mt-4">
            ...
            <Button 
              variant="outline" 
              click={() => document.getElementById('add-more-files')?.click()}
              disabled={selectedFiles.length >= maxFiles}
            >
              Add More Files
            </Button>
            <Button click={uploadEvidence} disabled={uploading || !evidenceTitle.trim()}>
              {#if uploading}
                Processing...
              {:else}
