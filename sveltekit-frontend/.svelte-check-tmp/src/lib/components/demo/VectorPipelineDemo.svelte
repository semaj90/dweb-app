...
<script lang="ts">
  ...
  onMount(() => {
    unsubscribe = vectorPipelineState.subscribe(value => {
      state = value;
    });
  });
  ...
  <Card>
    ...
    <CardContent>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <Button click={submitSingleJob} disabled={currentState === 'processingJob'}>
          Submit Single Job
        </Button>
        <Button click={submitBatchJobs} disabled={currentState === 'processingBatch'}>
          Submit Batch
        </Button>
        <Button click={runHealthCheck} variant="outline">
          Health Check
        </Button>
        <Button click={resetPipeline} variant="destructive">
          Reset Pipeline
        </Button>
        <Button click={enableWebGPU} disabled={pipelineStatus?.webgpu}>
          Enable WebGPU
        </Button>
        <Button click={disableWebGPU} disabled={!pipelineStatus?.webgpu}>
          Disable WebGPU
        </Button>
        <Button click={retryFailedJobs} disabled={!batchInfo?.failedJobs || batchInfo.failedJobs === 0}>
          Retry Failed
        </Button>
      </div>
