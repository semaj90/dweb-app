      ...
      {#if !isWorkflowActive && currentState.value === 'idle'}
        <Card>
          ...
          <CardContent>
            ...
            <Button click={startWorkflow} class="w-full">
              Start Custody Workflow
            </Button>
          </CardContent>
        </Card>
          ...
          <CardContent class="space-y-4">
            {#if currentState.value === 'awaitingApproval'}
              <div class="flex space-x-3">
                <Button click={approveWorkflow} variant="success">
                  Approve Custody
                </Button>
                <Button click={rejectWorkflow} variant="destructive">
                  Reject Custody
                </Button>
              </div>
            {/if}
            ...
            {#if currentState.value === 'collaboration'}
              <div class="flex space-x-3">
                <Button click={() => showTransferDialog = true} variant="outline">
                  Transfer Custody
                </Button>
                {#if !activeCollaborators.includes(userId)}
                  <Button click={joinCollaboration} variant="outline">
                    Join Collaboration
                  </Button>
                {:else}
                  <Button click={leaveCollaboration} variant="outline">
                    Leave Collaboration
                  </Button>
                {/if}
              </div>
            ...
            {#if currentState.value === 'error'}
              <div class="flex space-x-3">
                <Button click={retryWorkflow}>
                  Retry Workflow
                </Button>
                <Button click={cancelWorkflow} variant="destructive">
                  Cancel Workflow
                </Button>
              </div>
            {/if}
