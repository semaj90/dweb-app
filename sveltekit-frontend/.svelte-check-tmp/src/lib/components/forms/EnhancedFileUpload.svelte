...
<Card class="w-full">
  ...
  <CardContent class="space-y-6">
        ...
        <div class="grid gap-3">
          {#each selectedFiles as file (file.name)}
            <div class="flex items-center gap-3 p-3 border rounded-lg bg-muted bg-opacity-50">
              ...
              <Button variant="ghost" size="sm" click={() => removeFile(file.name)} disabled={isUploading} class="flex-shrink-0">
                <X class="h-4 w-4" />
              </Button>
            </div>
          {/each}
      ...
      <form class="space-y-6" on:submit|preventDefault={handleFormSubmit}>
        ...
        <div class="space-y-2">
          ...
          <div class="flex gap-2">
            ...
            <Button type="button" variant="outline" click={addTag} disabled={isUploading}>Add</Button>
          </div>
    ...
    <div class="flex justify-between items-center pt-4 border-t">
      ...
      <div class="flex gap-2">
        <Button variant="outline" click={() => oncancel?.()} disabled={isUploading}>Cancel</Button>

        <Button click={handleFormSubmit} disabled={selectedFiles.length === 0 || isUploading || Object.keys(errors).length > 0} class="min-w-24">
          {#if isUploading}
