        ...
        {#each columns as column (column.id)}
          <Card class="h-fit">
            ...
            <CardContent class="space-y-4">
              ...
              {#if column.id === "new"}
                <UploadZone
                  upload={(e) => handleFileUpload(e.detail, column.id)}
                />
              {/if}
              ...
              <div
                ...
                consider={(e) => handleDndConsider(e, column.id)}
                finalize={(e) => handleDndFinalize(e, column.id)}
              >
    ...
    {:else}
      ...
      >
        ...
        <div class="absolute bottom-4 right-4">
          <UploadZone
            minimal={true}
            upload={(e) => handleFileUpload(e.detail, "new")}
          />
        </div>
      </div>
    {/if}
...
{#if contextMenu.show}
  <ContextMenu.Root>
    ...
    >
      ...
      <ContextMenu.Item
        mouseenter={() => showMiniModal("citation")}
        mouseleave={hideMiniModal}
        select={() => saveTo("savedcitations")}
        >Add to /savedcitations</ContextMenu.Item
      >
      <ContextMenu.Item
        mouseenter={() => showMiniModal("aisummary")}
        mouseleave={hideMiniModal}
        select={() => saveTo("savedaisummaries")}
        >Add to /savedaisummaries</ContextMenu.Item
      >
      <ContextMenu.Item
        mouseenter={() => showMiniModal("userreport")}
        mouseleave={hideMiniModal}
        select={() => saveTo("saveduserreports")}
        >Add to /saveduserreports</ContextMenu.Item
      >
      <ContextMenu.Item
        mouseenter={() => showMiniModal("mcpcontext")}
        mouseleave={hideMiniModal}
        select={() => saveTo("mcpcontext")}
      ...
      <ContextMenu.Separator />
      <ContextMenu.Item
        mouseenter={() => showMiniModal("find")}
        mouseleave={hideMiniModal}
        select={findWithLLM}>Find (search/query with LLM)</ContextMenu.Item
      >
      {#if findModal.show}
        <div
        ...
        >
          <div class="flex flex-col gap-4">
            <div class="flex items-center gap-2">
              ...
              >
              <Button size="sm" variant="outline" click={closeFindModal}
                >Close</Button
              >
            </div>
