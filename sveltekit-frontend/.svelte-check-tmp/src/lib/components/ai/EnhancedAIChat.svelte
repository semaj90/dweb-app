...
<script lang="ts">
  ...
  let userAttention = writable({ focused: true, lastActivity: Date.now() });

  // Chat state
  let messages = writable<ChatMessage[]>([]);
  let sessionId = writable<string>('');
  ...
  let didYouMean = writable<string[]>([]);
  let processingMetrics = writable({
    responseTime: 0,
    tokenCount: 0,
    confidenceScore: 0,
    somCluster: -1,
  ...
  const {
    elements: { trigger: analysisTrigger, overlay, content: analysisContent, title, description, close },
    states: { open: analysisOpen }
  } = createDialog({
    forceVisible: true,
  });

  // Derived stores
  const lastUserMessage = derived(messages, ($messages) => 
    $messages.filter(m => m.role === 'user').slice(-1)[0] || null
  );

  const lastAIResponse = derived(messages, ($messages) => 
  ...
  async function initializeChat() {
    try {
      ...
      error.set(err instanceof Error ? err.message : 'Failed to initialize chat');
  ...
  function handleWebSocketMessage(data: any) {
    switch (data.type) {
      ...
      case 'streaming_token':
        ...
        streamingResponse.update(current => current + data.response.token);
        break;
      ...
      case 'analysis_result':
        ...
        currentAnalysis.set(data.analysis);
      ...
      case 'rag_context':
        // RAG context updated
        ragContext.set(data.context);
        recommendations.set(data.context.recommendations || []);
        didYouMean.set(data.context.did_you_mean || []);
        break;

      case 'typing_indicator':
        isTyping.set(data.typing);
        break;
    ...
    try {
      ...
      if (wsConnection && $isConnected) {
        ...
        if (result.ai_response) {
          messages.update(msgs => [...msgs, result.ai_response]);
          scrollToBottom();
        }

        // Update analysis and context
        if (result.user_message.analysis) {
          currentAnalysis.set(result.user_message.analysis);
        }

        if (result.user_message.context) {
          ragContext.set(result.user_message.context);
        }
      }
  ...
  async function generateReport() {
    try {
      ...
      error.set(err instanceof Error ? err.message : 'Failed to generate report');
    }
  }
  ...
  function setupAttentionTracking() {
    ...
    // Focus/blur tracking
    const focusHandler = () => {
      userAttention.update(current => ({ ...current, focused: true }));
      trackEvent('focus');
    };
    const blurHandler = () => {
      userAttention.update(current => ({ ...current, focused: false }));
      trackEvent('blur');
    };
    ...
    <div class="border-b border-border bg-card p-4">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          ...
          <!-- Generate Report -->
          <Button.Root variant="outline" size="sm" click={generateReport}>
            ...
            <div class="flex flex-wrap gap-2">
              {#each $didYouMean as suggestion}
                <Button.Root
                  variant="ghost"
                  size="sm"
                  click={() => applyDidYouMean(suggestion)}
                >
                  "{suggestion}"
                </Button.Root>
              {/each}
...
{#if selectedMessage}
  <Dialog.Root bind:open={showAnalysisDetails}>
    <Dialog.Content class="max-w-2xl">
      ...
      <Dialog.Footer>
        <Button.Root variant="outline" click={() => showAnalysisDetails = false}>
          Close
        </Button.Root>
      </Dialog.Footer>
    </Dialog.Content>
