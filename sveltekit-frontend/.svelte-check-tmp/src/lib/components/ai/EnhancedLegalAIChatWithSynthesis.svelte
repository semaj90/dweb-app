...
<div class="enhanced-legal-ai-chat flex flex-col h-full max-w-6xl mx-auto {class}">
      ...
      <div class="message-bubble {message.role}" transitifly={{ y: 20, duration: 300 }}>
        <div class="flex items-start gap-3">
          ...
          <div class="flex-shrink-0 flex flex-col gap-1">
            <Button variant="ghost" size="sm" click={() => copyToClipboard(message.content)}>
  ...
  <div class="mt-4 flex gap-2">
    <Input
      ...
      class="flex-1" />
    <Button click={sendMessage} disabled={!currentInput.trim() || isProcessing}>
      {#if isProcessing}
        <Loader2 class="w-4 h-4 animate-spin" />
      {:else}
        <Send class="w-4 h-4" />
    ...
    <Card class="mt-4" transitifly={{ y: 20, duration: 300 }}>
      <CardHeader>
        <CardTitle class="flex items-center justify-between">
          Detailed Analysis
          <Button variant="ghost" size="sm" click={() => (showAdvancedAnalysis = false)}>
            ×
          </Button>
        </CardTitle>
      </CardHeader>
