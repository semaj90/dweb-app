...
export class GpuAcceleratedJsonParser {
     ...
     */
    async parseWithWorker(json, options = {}) {
        if (!this.webWorker) {
     ...
     */
    async parseBatch(jsonArray, options = {}) {
        await this.ensureInitialized();
    ...
    async validateWithGpu(json) {
        ...
        try {
            ...
            passEncoder.end();
            commandEncoder.copyBufferToBuffer(outputBuffer, 0, stagingBuffer, 0, jsonBytes.length * 4);
            ...
            for (let i = 0; i < resultArray.length; i++) {
                if (resultArray[i] === 0) {
                    errors.push(`Invalid character at position ${i}: ${String.fromCharCode(jsonBytes[i])}`);
                }
            }
            stagingBuffer.unmap();
            return {
     ...
     */
    async benchmark(testJson, iterations = 100) {
        await this.ensureInitialized();
 ...
 */
export function getGlobalJsonParser() {
    if (!globalParser) {
 ...
 */
export async function parseJson(json, options) {
 ...
 */
export async function parseJsonBatch(jsonArray, options) {
    const parser = getGlobalJsonParser();
