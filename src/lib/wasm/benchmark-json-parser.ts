...
export class JsonParserBenchmark {
  ...
  private testData: { [key: string]: string } = {};
  ...
  private async runBenchmark(
    name: string,
    parseFunction: (json: string) => Promise<any> | any,
    testJson: string,
  ...
  async comparePerformance(
    ...
    console.log(
      `🏃 Running comparison benchmark: ${testName} (${iterations} iterations)`
    );
    ...
    const wasmResult = await this.runBenchmark(
      `WASM ${testName}`,
      (json) => this.parser.parse(json, { useCache: true }),
      testJson,
      iterations
    );

    // Benchmark native JSON.parse
    const nativeResult = await this.runBenchmark(
      `Native ${testName}`,
      JSON.parse,
      testJson,
      iterations
    ...
    const efficiency =
      (wasmResult.operationsPerSecond / nativeResult.operationsPerSecond) * 100;
   ...
   */
  async runFullBenchmark(): Promise<{ [testName: string]: ComparisonResult }> {
    console.log("🚀 Starting comprehensive JSON parser benchmark...");

    const results: { [testName: string]: ComparisonResult } = {};
   ...
   */
  async benchmarkBatchParsing(): Promise<{
    ...
    const wasmBatch = await this.runBenchmark(
      "WASM Batch",
      (jsonArray) =>
        this.parser.parseBatch(jsonArray as any, { useWorker: true }),
      batchData as any,
      50
    );
   ...
   */
  async benchmarkGpuValidation(): Promise<{
    gpuValidation: BenchmarkResult;
    cpuValidation: BenchmarkResult;
    ...
    const gpuValidation = await this.runBenchmark(
      "GPU Validation",
      (json) => this.parser.validateWithGpu(json),
      testJson,
      20
    );

    // Benchmark CPU validation (fallback)
    const cpuValidation = await this.runBenchmark(
      "CPU Validation",
      (json) => this.parser.validate(json),
      testJson,
      20
    );
  ...
  async benchmarkCacheEffectiveness(): Promise<{
    ...
    const withoutCache = await this.runBenchmark(
      "Without Cache",
      (json) => this.parser.parse(json, { useCache: false }),
      testJson,
      200
    );
   ...
   */
  generateReport(results: { [testName: string]: ComparisonResult }): string {
   ...
   */
  async runStressTest(iterations: number = 1000): Promise<{
    memoryGrowth: number;
    averageTime: number;
    errors: number;
...
export async function runBenchmark(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `- WASM batch time: ${batchResults.wasmBatch.averageTime.toFixed(2)}ms`
    );
    console.log(
      `- Sequential time: ${batchResults.sequential.averageTime.toFixed(2)}ms\n`
    );
    ...
    console.log(
      `- GPU time: ${gpuResults.gpuValidation.averageTime.toFixed(2)}ms`
    );
    console.log(
      `- CPU time: ${gpuResults.cpuValidation.averageTime.toFixed(2)}ms\n`
    );

    console.log("### Cache Effectiveness");
    console.log(`- Cache speedup: ${cacheResults.speedup.toFixed(2)}x`);
    console.log(
      `- With cache: ${cacheResults.withCache.averageTime.toFixed(2)}ms`
    );
    console.log(
      `- Without cache: ${cacheResults.withoutCache.averageTime.toFixed(2)}ms\n`
    );
    ...
    console.log(
      `- Memory growth: ${(stressResults.memoryGrowth / 1024).toFixed(2)}KB`
    );
    console.log(`- Average time: ${stressResults.averageTime.toFixed(2)}ms`);
    console.log(`- Errors: ${stressResults.errors}`);
    console.log(
      `- Cache efficiency: ${(stressResults.cacheEfficiency * 100).toFixed(1)}%`
    );
