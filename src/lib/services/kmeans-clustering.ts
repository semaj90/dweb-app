...
export class LegalKMeansClusterer {
  ...
  async fit(embeddings: number[][]): Promise<ClusterResult[]> {
    console.log(`Starting K-Means clustering with k=${this.config.k} on ${embeddings.length} documents...`);
    
    if (embeddings.length < this.config.k) {
      throw new Error(`Cannot cluster ${embeddings.length} documents into ${this.config.k} clusters`);
    }
  ...
  private updateCentroids(embeddings: number[][]): void {
    ...
    for (let k = 0; k < this.config.k; k++) {
      const clusterPoints = embeddings.filter((_, index) => this.labels[index] === k);
   ...
   */
  private hasConverged(currentLabels: number[], previousLabels: number[]): boolean {
   ...
   */
  private averageIntraClusterDistance(embeddings: number[][], pointIndex: number): number {
    const clusterLabel = this.labels[pointIndex];
    const clusterPoints = embeddings.filter((_, index) => 
      this.labels[index] === clusterLabel && index !== pointIndex
    );
   ...
   */
  private averageNearestClusterDistance(embeddings: number[][], pointIndex: number): number {
   ...
   */
  private async generateClusterResults(embeddings: number[][]): Promise<ClusterResult[]> {
    ...
    for (let k = 0; k < this.config.k; k++) {
      ...
      const clusterEmbeddings = clusterIndices.map(index => embeddings[index]);
      const centroid = this.centroids[k];
      
      // Calculate cluster coherence
      const coherenceScore = this.calculateClusterCoherence(clusterEmbeddings, centroid);
      
      // Create document clusters with similarity scores
      const documents: DocumentCluster[] = clusterIndices.map(index => ({
        documentId: `doc_${index}`,
        similarity: 1 - (this.euclideanDistance(embeddings[index], centroid) / Math.sqrt(centroid.length)),
      ...
      results.push({
        ...
        documents,
        coherenceScore,
        somPosition: { x: 0, y: 0 } // Will be updated by SOM integration
      });
    }
   ...
   */
  private calculateClusterCoherence(clusterEmbeddings: number[][], centroid: number[]): number {
  ...
  async analyzeLegalClusters(
    ...
    for (let k = 0; k < this.config.k; k++) {
      ...
      const clusterMetadata = clusterIndices.map(index => documentMetadata[index]);
      
      // Extract legal topics
      const legalTopics = this.extractTopics(clusterMetadata.map(m => m.keywords).flat());
  ...
  private assessRiskLevel(metadata: Array<{ type: string; keywords: string[] }>, silhouetteScore: number): 'low' | 'medium' | 'high' {
    const riskKeywords = ['liability', 'penalty', 'violation', 'dispute', 'litigation'];
    const riskCount = metadata.reduce((count, doc) => 
      count + doc.keywords.filter(keyword => riskKeywords.includes(keyword.toLowerCase())).length, 0
    );
    
    if (riskCount > metadata.length * 0.3 || silhouetteScore < 0.3) return 'high';
    if (riskCount > metadata.length * 0.1 || silhouetteScore < 0.5) return 'medium';
    return 'low';
  }
  
  private generateRecommendations(topics: string[], types: string[], riskLevel: string): string[] {
  ...
  static async loadFromRedis(redis: Redis): Promise<LegalKMeansClusterer | null> {
