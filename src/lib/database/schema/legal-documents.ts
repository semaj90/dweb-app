...
export const legalDocuments = pgTable('legal_documents', {
  ...
  contentEmbeddingIdx: index('legal_documents_content_embedding_idx').using('hnsw', table.contentEmbedding).with({ 'm': 16, 'ef_construction': 64 }),
  titleEmbeddingIdx: index('legal_documents_title_embedding_idx').using('hnsw', table.titleEmbedding).with({ 'm': 16, 'ef_construction': 64 }),
...
export const legalEntities = pgTable('legal_entities', {
  ...
  nameEmbeddingIdx: index('legal_entities_name_embedding_idx').using('hnsw', table.nameEmbedding).with({ 'm': 16, 'ef_construction': 64 }),
}));
